...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(255, 0, 0);
		triangle(45, 105, 50, 97, 55, 105);
		fill(255, 255, 255);
		ellipse(50, 125, 10, 10);
		fill(255, 255, 255);
		rect(45, 143, 10, 10);
		fill(255, 255, 0);
		ellipse(50, 194, 10, 10);
		fill(0, 0, 255);
		ellipse(50, 217, 10, 10);
		fill(255, 0, 0);
		...
		**/
		...
		text("Landquake", 75, 123);
		text("Seaquake", 75, 146);
		text("Size ~ Magnitude", 45, 169);
		text("Shallow", 75, 192);
		text("Intermediate", 75, 215);
		text("Deep", 75, 238);
		text("Past Week", 75, 261);
	}
	...
	{	
		...
		int seaquake_counter = 0;
		int tot_earthquakecounter = 0;
		for (Marker cm : countryMarkers) {
			//initialize a quake counter inside the loop
			int earthquake_counter = 0;
			String name_property = (String)cm.getProperty("name"); //name property of country marker
			//System.out.println("name_property: " + name_property);
			//loop through all of the earthquake markers
			for (Marker quake : quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker)quake;
				String country_property = (String)em.getProperty("country");
				if (em.isOnLand() == true && country_property.equals(name_property)) {
					earthquake_counter += 1;
				} 
				/**
				else if (em.isOnLand() == false) {
					seaquake_counter += 1;
				}
				**/
				
			}
			if (earthquake_counter >= 1) {
				System.out.println(name_property + ": " + earthquake_counter);
				tot_earthquakecounter += earthquake_counter;
			}
		}
		int tot_quakes = quakeMarkers.size();
		seaquake_counter = tot_quakes - tot_earthquakecounter;
		System.out.println("Number of seaquakes: " + seaquake_counter);
